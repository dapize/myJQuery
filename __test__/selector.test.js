...
describe('Selector', () => {
  ...
  test('jQuery Node', () => {
    ...
    $nodejQuery.otherProperty = "I'm other property";
    expect($nodejQuery).not.toBe($cloneNodeJQ);
  ...
  test('Tag and Class Name With Context', () => {
    ...
    const prevObj = {0: section, length: 1};
    const divsObj = {0: divs[0], 1: divs[1], 2: divs[2], length: 3}
    expect($divs).toEqual(Object.assign(divsObj, {prevObject: prevObj}));
    expect($items).toEqual(Object.assign(divsObj, {prevObject: prevObj}));

    // by class with context node
    const main = document.querySelector('main');
    const item2 = document.querySelectorAll('.item-2');
    const objMain = {prevObject: {0: main, length: 1}}
    const objItem2 = {0: item2[0], 1: item2[1], length: 2};
    const $item2 = $('.item-2', main);
    expect($item2).toEqual(Object.assign(objItem2, objMain));

    // by class with context node jQuery
    const blocks = document.querySelectorAll('.block');
    const objBlocks = {0: blocks[0], 1: blocks[1], length: 2, prevObject: {0: document, length: 1}}
    const $blocks = $('.block');
    const $item2b = $('.item-2', $blocks);
    expect($item2b).toEqual(Object.assign(objItem2, {prevObject: objBlocks}));
  ...
  test('Node DOM', () => {
    ...
    const $div = $(div);
    expect($div).toEqual({0: div, length: 1});
  });
  ...
  test('Nodes DOM', () => {
    ...
    const items = document.querySelectorAll('.item');
    const $items = $(items);
    expect($items).toEqual({0: items[0], 1: items[1], 2: items[2], length: 3});
  });
})